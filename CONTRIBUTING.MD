...
- Собрать все пакеты: `yarn build`
- Прогнать тесты монорепозитория: `yarn test`
- Проверить линт: `yarn lint`
- Локально собрать пакет `arui-scripts`: `yarn workspace arui-scripts build`

## Изменения в `arui-scripts`
Перед PR проверьте в целевом приложении:
1. `arui-scripts doctor`
2. `arui-scripts build`
3. `arui-scripts test`

Если вы меняете поведение CLI, то необходимо обновить документацию:
- `packages/arui-scripts/docs/commands.md`
- `packages/arui-scripts/README.md`
