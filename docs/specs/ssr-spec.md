...
## 10. Addendum: CSS delivery for module-federation modules

Discovered during real-world usage of the implemented spec: for `mountMode: 'default'`
modules, `getModuleResources` returns `scripts: ['assets/remoteEntry.js'], styles: []` — CSS
chunks are loaded by the rspack runtime only after `remoteEntry.js` executes on the client.
The SSR host therefore has no style URLs to emit, and server-rendered module HTML paints
unstyled until the client loads and runs the module. This defeats the no-FOUC goal for MF
modules (compat modules are unaffected: their CSS is already attributed in the manifest).

The per-expose CSS **already exists** as separate chunk files (e.g.
`assets/wmf-src_modules_server-state-module_index_tsx.css` under the unnamed `""` entry of
`webpack-assets.json`); it is just not attributed to module ids —
`processAssetsPluginOutput` fabricates MF module entries with `js: remoteEntry.js` only.

### Fix — three coordinated changes

1. **`arui-scripts` (build): attribute per-expose CSS in the app manifest.** A plugin walks
   the chunk graph from each expose declared in `configs.modules.exposes` (the container
   entry's blocks), collects CSS assets of the expose's chunk group plus its synchronous
   descendants, and writes them into the module's manifest entry:
   `{ mode: 'default', js: 'remoteEntry.js', css: [...] }`. Chunk-graph walking, not chunk-name
   matching (names like `wmf-src_..._index_tsx.css` are an implementation detail). The
   `AruiAppManifest` type already permits per-module `css` — purely additive. CSS of
   **async** chunks inside the expose is intentionally excluded: lazily-loaded components are
   not part of the SSR HTML (they render Suspense fallbacks on the module server) and keep
   loading their CSS client-side as today.

2. **`@alfalab/scripts-server`: populate `styles` for default-mode modules.**
   `createGetModulesMethod` reads the manifest CSS for the module — **only when the request
   carries the `ssr` field**, preserving the byte-for-byte response guarantee (section 7,
   item 6) for existing non-SSR consumers.

3. **`@alfalab/scripts-modules`: make the rspack runtime adopt server-emitted styles.**
   The CSS-loading runtime generated by `CssExtractRspackPlugin` (a port of
   mini-css-extract-plugin) checks for already-present stylesheets before fetching a CSS
   chunk, matching both `<link>` by `href`/`data-href` and **`<style data-href="...">`** —
   a hook that exists precisely for SSR-inlined CSS. The inline `<style>` tags emitted by
   `createSsrMounter` (section 5.4) therefore additionally carry
   `data-href={resolvedCssUrl}`. When the MF runtime later loads the expose's chunk it finds
   the style tag, marks the CSS chunk as loaded, and fetches nothing → no double-load, no
   flash, no runtime patching. With `stylesMode: 'link'` no extra attribute is even needed —
   the runtime matches server-emitted `<link>` tags by plain `href` — but `data-href` is still
   set for uniformity of the adoption logic.

### Ownership rules for MF module styles

Runtime-inserted MF stylesheets never carried `data-parent-app-id`, so
`removeModuleResources` has never removed them, and the runtime's "CSS chunk is loaded"
bookkeeping assumes they persist. SSR-emitted styles for default-mode modules must follow the
same rule: marked with `data-href` + `data-module-ssr-href` but **not** `data-parent-app-id`.
Unmount leaves them in place (consistent with today's MF behavior); a remount in the same
session finds runtime bookkeeping and DOM in agreement. Compat-module SSR styles keep the
section 5.4/6.2 behavior unchanged (`data-parent-app-id` present, removed on unmount).

### URL matching caveat

The runtime compares `data-href` against both the chunk path (`assets/....css`) and
`publicPath + path`. With `publicPath: 'auto'` the full href resolves from the remoteEntry
script's origin — the same `baseUrl` the host used via `resolveResourceUrl`, so absolute URLs
match in the standard setup. Setups that rewrite asset URLs between the manifest and delivery
(CDN rewrites, versioned-bundle proxying) can break the match; the failure mode is a duplicate
CSS load, not broken styles. Document this; emit the absolute resolved URL in `data-href`.
...
Ships as its own phase (all three packages, one changeset each): manifest attribution plugin →
`createGetModulesMethod` change → `data-href` emission + ownership rules in `createSsrMounter`
and the adoption logic. Independent of and after Phases 0–4.
