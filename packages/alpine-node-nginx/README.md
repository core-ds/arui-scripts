...
```sh
docker build --build-arg NODE_VERSION=24.14.1 --build-arg ALPINE_VERSION=3.22 -t alfabankui/arui-scripts:test .
```
Или для slim версии:
```sh
docker build --build-arg NODE_VERSION=24.14.1 --build-arg ALPINE_VERSION=3.22 -t alfabankui/arui-scripts:test -f Dockerfile-slim .
```
...
```sh
docker buildx build --platform linux/amd64 --build-arg NODE_VERSION=24.14.1 --build-arg ALPINE_VERSION=3.22 -t alfabankui/arui-scripts:test -f Dockerfile-slim --load .
```
