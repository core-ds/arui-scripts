...
/**
 * start.sh — entrypoint образа. Для serverful-режима поднимает nginx + nodejs, для clientOnly —
 * подставляет env-config и запускает nginx.
 */
export function renderStartScript(config: ResolvedDockerConfig): string {
