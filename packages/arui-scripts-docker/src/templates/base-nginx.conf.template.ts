...
/**
 * http-блок nginx-конфига (базовый nginx.conf). Значения по умолчанию можно переопределить через
 * `nginx` в опциях сборки.
 */
export function renderBaseNginxConf(config: ResolvedDockerConfig): string {
    const nginx = {
        ...baseNginxConfig,
        ...config.nginx,
    };
