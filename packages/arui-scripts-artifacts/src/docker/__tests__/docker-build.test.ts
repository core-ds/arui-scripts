...
import { getDockerBuildCommand, getPlatformFlag } from '../build-command';
import { applyCommandLineArguments, getBuildParams } from '../build-params';
...
describe('applyCommandLineArguments', () => {
    ...
    it('should not mutate the source config', () => {
        const config = resolveArtifactsConfig(baseOptions);

        applyCommandLineArguments(config, ['registry=r.example.com']);
...
describe('getPlatformFlag', () => {
    it('should return empty string when platform is false', () => {
        const config = resolveArtifactsConfig({ ...baseOptions, docker: { platform: false } });
...
describe('getDockerBuildCommand', () => {
    ...
    it('should include extra build args', () => {
        const config = resolveArtifactsConfig({
            ...baseOptions,
            docker: { platform: false, buildArgs: { COMMIT_SHA: 'abc123' } },
        });
    ...
    it('should escape extra build args so they cannot break out of the command', () => {
        const config = resolveArtifactsConfig({
            ...baseOptions,
            docker: { platform: false, buildArgs: { EVIL: 'a"; touch /tmp/pwned; #' } },
        });
    ...
    it('should escape image name and context', () => {
        const config = resolveArtifactsConfig({
            ...baseOptions,
            name: 'app; rm -rf /',
            docker: { platform: false, context: './my context' },
        });
