...
describe('renderTemplates', () => {
    it('should render runtime dockerfile with the base image and start.sh', () => {
        const config = resolveArtifactsConfig({ ...baseOptions, baseDockerImage: 'my/base:1.0.0' });
    ...
    it('should render compiled dockerfile with install command', () => {
        const config = resolveArtifactsConfig({ ...baseOptions, yarnVersion: 'unavailable' });
    ...
    it('should not render base nginx conf when nginx is disabled', () => {
        const config = resolveArtifactsConfig({ ...baseOptions, nginx: false });
    ...
    it('should render base nginx conf with custom worker processes', () => {
        const config = resolveArtifactsConfig({ ...baseOptions, nginx: { workerProcesses: 9 } });
        const templates = renderTemplates({ config });
    ...
    it('should use compiled dockerfile when the variant comes from the config', () => {
        const config = resolveArtifactsConfig({ ...baseOptions, variant: 'compiled' });
        const templates = renderTemplates({ config });
