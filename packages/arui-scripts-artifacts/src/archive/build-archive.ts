...
import { type ArtifactsOptions, type ResolvedArtifactsConfig } from '../config/types';
import { NGINX_CONFIG_FILENAME } from '../nginx/constants';
import { type BeforeBuildHook, runHostPipeline } from '../pipeline/host-pipeline';
import { renderTemplates } from '../pipeline/render';
 ...
 * `package.json` и дополнительные директории из `archive.additionalPaths`.
 *
...
export async function buildArchive(options: BuildArchiveOptions = {}): Promise<void> {
    ...
    const { cwd, buildPath, archive, localFiles } = config;
    const pathToTempDir = path.join(cwd, archive.tempDirName);

    try {
        console.log(`Build archive ${archive.name}`);
        ...
        await Promise.all([
            ...
            ...archive.additionalPaths.map((additionalPath) =>
                fs.copy(path.join(cwd, additionalPath), path.join(pathToTempDir, additionalPath)),
            ),
        ]);

        await createTar({ file: archive.name, cwd: pathToTempDir }, fs.readdirSync(pathToTempDir));
