...
Для серверных модулей мы не предоставляем готовой функции, так как у вас могут быть разные требования к тому, как
формировать запросы к серверу (добавление заголовков, авторизация и т.д.). В этом случае вы можете использовать функцию, возвращающую
настройки для вашей собственной реализации `getModuleResources`:

```ts
import { createModuleLoader, getServerFetcherParams } from '@alfalab/scripts-modules';
import { response } from 'express';

const fetcher = (params) => {
    const { method, relativePath } = getServerFetcherParams();

    return fetch(`http://localhost:8081${relativePath}`, {
        method,
        headers: {
            'Content-Type': 'application/json',
            // ...другие ваши кастомные заголовки
        },
...
const MyComponent = () => {
    const { loadingState, targetElementRef } = useModuleMounter(loader); // вторым параметром можно передать параметры, которые будут переданы в getModuleResources
