...
Браузер, поддерживающий compression dictionary transport будет запрашивать ресурсы новой версии если они попадают под маску из `Use-As-Dictionary` заголовка:
...
Сервер проверит заголовок dictionary-id, найдет по нему версию файла сжатую словарем, проверит подпись файла (содержится в заголовке `Available-Dictionary`)
и ответить контентом, сжатым с кастомным словарем.
...
Маска для заголовка `Use-As-Dictionary` должна полностью совпадать с реальным адресом, по которому запрос делает браузер.
Поэтому если ваше приложение находится за прокси, меняющим реальный адрес приложения необходимо убедиться что прокси добавляет заголовок `x-forwarded-prefix`.
