...
собранный файл `.build/server.js`. Всё это сделано с помощью добавления их в исключение
(см. `src/configs/webpack.server.ts`)
...
Но когда вы руками меняете код внутри node_modules - вы не сможете увидеть ваши изменения, так как сборщик будет
использовать данные из кеша. Для того чтобы ваши изменения применились в сборке вам необходимо очистить кеш
rspack и перезапустить сборку. Кеши хранятся в папке `.cache` внутри `node_modules` вашего проекта.
