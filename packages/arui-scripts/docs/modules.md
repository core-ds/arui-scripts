...
Если что-то пойдет не так — есть [расширение для отладки модулей](devtools.md): вкладка **ARUI**
в Chrome DevTools показывает по каждому модулю адрес, версию, стадии загрузки с таймингами,
содержимое share scope и текст ошибки, если модуль не загрузился.

# Пощупать вживую

В репозитории `arui-scripts` лежит работающая пара «провайдер + потребитель», на которой можно
посмотреть все способы подключения сразу, ничего не настраивая. Провайдер поднимаем первым:
за его манифестом потребитель идёт в момент, когда монтирует модуль, и на неподнятом провайдере
вы увидите вместо модуля ошибку загрузки.

```bash
yarn workspace example-modules start
```
...
Потребитель откроется на [localhost:8080](http://localhost:8080), провайдер живёт
на [localhost:8082](http://localhost:8082). На странице потребителя есть блок «Примеры разных
способов подключения модулей» — по вкладке на каждый способ:

| Вкладка                       | Что показывает                                        | Раздел документации                                             |
| ----------------------------- | ----------------------------------------------------- | --------------------------------------------------------------- |
| `Default module`              | обычный модуль через module federation                | [Как использовать](#как-использовать)                             |
| `Compat module`               | тот же модуль в режиме `compat`                       | [Сравнение способов подключения](#сравнение-способов-подключения-модулей) |
| `Server state module`         | модуль с серверной частью, отдающей состояние         | [Управление модулями с сервера](#возможность-управления-модулями-с-сервера) |
| `Server state compat module`  | то же самое в режиме `compat`                         | там же                                                            |
| `Abstract module`             | немонтируемый модуль, отдающий только функции         | [Абстрактные модули](#абстрактные-модули)                         |
| `Server state Factory Module` | модуль-фабрика с состоянием с сервера                 | [Модули-фабрики](#модули-фабрики)                                 |
| `Factory Compat module`       | модуль-фабрика в режиме `compat`                      | там же                                                            |

Что смотреть в коде:

-   `packages/example-modules/arui-scripts.config.ts` — как провайдер объявляет `modules.exposes`,
    `compatModules.exposes`, `shared` и `cssPrefix`;
-   `packages/example-modules/src/modules/*` — входные точки самих модулей;
-   `packages/example/src/module-mounters/*` — как потребитель подключает каждый тип:
    `createModuleLoader`, `createModuleFetcher`, `useModuleMounter`.

Диагностика там собирается сама: поставьте [расширение](devtools.md), откройте вкладку **ARUI**
в DevTools и переключайте вкладки примера — в таблице будет видно, что грузится, сколько заняла
каждая стадия и что попадает в share scope.

**Важная оговорка.** `packages/example` — это ещё и тестовый стенд самого `arui-scripts`, поэтому
рядом с модулями там живут проверки postcss, счётчики hot-loader, вебворкер и прочее, к модулям
отношения не имеющее. Копировать оттуда стоит только то, что перечислено выше, — остальное
специфично для тестирования сборщика.
