...
Если что-то пойдет не так — в dev-сборке доступна [панель отладки модулей](devtools.md) по
`Ctrl/Cmd+Shift+M`. Она показывает по каждому модулю адрес, версию, стадии загрузки с таймингами,
...
Панель отладки там уже включена: откройте `Ctrl/Cmd+Shift+M` и переключайте вкладки примера —
в таблице будет видно, что грузится, сколько заняла каждая стадия и что попадает в share scope.
