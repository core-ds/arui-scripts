...
#### devtools
Управляет тем, попадёт ли в клиентский бандл панель отладки модулей. По умолчанию `'dev'`.

- `'dev'` - подмешивать только в dev-сборку, в прод-бандле кода панели нет вовсе;
- `'always'` - подмешивать и в прод-сборку: так панель можно включить на стенде;
- `'off'` - не подмешивать никогда.

Настройка управляет только сборкой. Видимостью панели управляет ключ `arui:devtools` в
`localStorage`, поэтому в прод-сборке с `'always'` панель молчит, пока её не включат руками.
Подробнее в разделе [панель отладки модулей](devtools.md).
