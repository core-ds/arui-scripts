...
> [документации пакета](../../arui-scripts-artifacts/docs/migration.md)).
...
Цели компиляции для `swc` берутся из списка поддерживаемых браузеров (browserslist-конфиг проекта или [`supporting-browsers`](overrides.md#тонкая-настройка)),
для серверного кода и тестов - из `supportingNode`. Хелперы SWC не дублируются в каждом модуле, а импортируются из `@swc/helpers`:
пакет идёт в зависимостях `arui-scripts`, а если из проекта он не резолвится (например, при pnpm без hoisting), `arui-scripts` подставит свою копию через `resolve.alias`.
...
#### nodeExternals
Настройки для `webpack-node-externals`, см. [документацию](https://www.npmjs.com/package/webpack-node-externals). По умолчанию `{ modulesFromFile: true }`.

`allowlist` настраивается через систему [оверайдов](overrides.md), свойство `serverExternalsExemptions`
