...
Точка входа для полифилов. Будет подключаться до основной клиентской точки входа (можно передать одну строку или массив путей или не задавать, тогда отдельный бандл полифилов не добавляется).

По умолчанию ключ не задан: для многих проектов отдельный файл не нужен. Если нужны полифилы при `codeLoader: 'swc'` или Babel с `useBuiltIns: 'entry'`, обычно создают файл с `import 'core-js/stable'` и указывают путь в этом поле (пакет `core-js` должен быть в зависимостях приложения). Пример для новых проектов см. в [README](../README.md).
...
Код из `node_modules` идет **по другой ветке**:
тоже через `swc-loader`, но без того же уровня транспиляции под старые браузеры, что для вашего проекта, поэтому современный синтаксис из зависимостей **может оказаться в бандле** почти без изменений. То, что вы задаете в оверрайдах `browsers` / `supportingBrowsers` и тд., в первую очередь относится к **вашему** коду проекта, но никак не гарантирует такую же обработку всего вашего кода из `node_modules`.

**Когда использовать `codeLoader: 'babel'`**

Если нужно поддерживать старые браузеры и чтобы к зависимостям применялась та же логика обработки/транспиляции кода, что и к вашему проекту, то используйте `codeLoader: 'babel'`.
Тогда и ваш проект и `node_modules` пройдут через `babel-loader`. Для зависимостей используется **`babelDependencies`** и **`@babel/preset-env`** по [browserslist](https://github.com/browserslist/browserslist) проекта.

**Исходите из вашей ситуации на проекте:**
**`swc`** - более быстрая скорость сборки, старые браузеры не критичны, допустимо, что «под браузеры» в основном приводится **ваш код проекта**, а не код из `node_modules`. Также **`swc` обязателен** для опции `experimentalReactCompiler` (см. ниже)

**`babel`** - нужно, чтобы и код проекта и код из `node_modules` транспилировались одинаково.

Точечная настройка отдельных пакетов при `swc` — в [overrides](./overrides.md) (например `webpackClient`).
