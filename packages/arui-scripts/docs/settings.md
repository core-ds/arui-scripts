...
Отключает проверку типов вебпаком в дев режиме. Когда настройка включена, tsc будет запускаться отдельным процессом, и будет только
писать ошибки в консоль, но не останавливать сборку. По умолчанию `false`.
