...
#### previousVersionPath
Путь до папки/папок с асетами предыдущей версии приложения. По умолчанию `undefined`.

Используется для сжатия ассетов с поддержкой [compression dictionary transport](./compression-dictionary).

Если передан - при сборке кроме `.gz` и `.br` версий статических файлов будет также формировать `.dcb` файлы.
