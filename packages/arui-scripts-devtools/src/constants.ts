...
/**
 * Ключ в `window` документа панели, через который страница devtools сообщает ей,
 * показана вкладка или спрятана.
 *
 * Оба документа - наши, одного расширения, поэтому мост тут прямой, без postMessage.
 * Ходит он в одну сторону: `panel.onShown`/`onHidden` есть только у страницы devtools,
 * а поллинг живёт в панели.
 */
export const PANEL_BRIDGE_KEY = '__ARUI_DEVTOOLS_PANEL__';
